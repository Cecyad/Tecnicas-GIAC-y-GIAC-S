...
def orden(cadenaa):
    ...
    nume=0
    while nume < medida-1:
        lista1=set(cadenaa[nume])
        lista2=set(cadenaa[nume+1])
        final=lista1 & lista2

        if len(final)>0:
            lista3=lista2-lista1
            result=cadenaa[nume]+list(lista3)
            cadenass.append(result)
            nume+=2
            bandera=1
        else:
            cadenass.append(cadenaa[nume])
            nume+=1
            if nume==medida-1:
                cadenass.append(cadenaa[nume])
    '''
...
if aa==1:
    subgrupoprueba=pd.read_csv('test1.csv',header=None)
if aa==2:
    subgrupoprueba=pd.read_csv('test2.csv',header=None)
if aa==3:
    subgrupoprueba=pd.read_csv('test3.csv',header=None)
if aa==4:
    subgrupoprueba=pd.read_csv('test4.csv',header=None)
if aa==5:
    subgrupoprueba=pd.read_csv('test5.csv',header=None)
...
x_train=df[[0,1,2,3,4,5,6,7]]
y_train=df[[8]]
x_test=df2[[0,1,2,3,4,5,6,7]]
y_test = df2[[8]]
...
train_x=pd.DataFrame(trainn[[0,1,2,3,4,5,6,7]])
train_y=pd.DataFrame(trainn[[8]])

test_x=pd.DataFrame(subgrupoprueba[[0,1,2,3,4,5,6,7]])
test_y=pd.DataFrame(subgrupoprueba[[8]])
